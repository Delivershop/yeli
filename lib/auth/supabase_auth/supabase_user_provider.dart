...
class YeliSupabaseUser extends BaseAuthUser {
  YeliSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> yeliSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = YeliSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
